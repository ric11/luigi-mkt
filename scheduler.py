...
class scheduler(Config):
   ...
   # remove_delay = parameter.FloatParameter(default=600.0)     #custom ricard
...
class SimpleTaskState(object):
    ...
    def update_status(self, task, config):
     ...
     #   if (not task.stakeholders) and (task.remove is None) and (task.status != RUNNING): # custom ricard
            # We don't check for the RUNNING case, because that is already handled
            # by the fail_dead_worker_task function.
           # logger.debug("Task %r has no stakeholders anymore -> might remove " # custom ricard
          #               "task in %s seconds", task.id, config.remove_delay) # custom ricard
